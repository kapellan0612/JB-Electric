// Services.jsx — JB Electric Services v2 (Bold & Modern)
const SERVICES = [
  {
    id: 'installations',
...
const Services = ({ setScreen }) => {
  const [active, setActive] = React.useState(null);
  const [hovered, setHovered] = React.useState(null);

  return (
    <div style={svcStyles.root}>
      {/* ── Section header ── */}
      <div style={svcStyles.header}>
        <div style={svcStyles.headerLeft}>
          <div style={svcStyles.eyebrow}>
            <span style={svcStyles.eyebrowLine}/>
            What We Do
          </div>
          <h2 style={svcStyles.title}>Expert Electrical<br/>Services</h2>
        </div>
        <div style={svcStyles.headerRight}>
          <p style={svcStyles.sub}>Residential and commercial electrical work done right — on time, on budget, to code. Licensed and fully insured for your peace of mind.</p>
          <button
            className="jb-btn-primary jb-btn-inverse"
            style={svcStyles.headerCta}
        ...
        {SERVICES.map((svc, idx) => {
          ...
          return (
            <div
              ...
              style={{
                ...svcStyles.card,
                ...(isActive ? svcStyles.cardActive : {}),
              }}
              onClick={() => setActive(isActive ? null : svc.id)}
            >
              <div style={svcStyles.cardTop}>
                <div style={{ ...svcStyles.num, ...(isActive ? { color: 'rgba(255,208,14,0.25)' } : {}) }}>{svc.number}</div>
                <div className="jb-icon-wrap" style={{ ...svcStyles.iconWrap, ...(isActive ? svcStyles.iconWrapActive : {}) }}>
                  {svc.icon}
                </div>
              </div>
              <h3 style={{ ...svcStyles.cardTitle, ...(isActive ? { color: '#fff' } : {}) }}>{svc.title}</h3>
              <p style={{ ...svcStyles.cardText, ...(isActive ? { color: 'rgba(255,255,255,0.65)' } : {}) }}>
                {isActive ? svc.long : svc.short}
              </p>
              <div style={{ ...svcStyles.cardFooter, ...(isActive ? { borderTopColor: 'rgba(255,208,14,0.15)' } : {}) }}>
                {isActive ? (
                  <button
                    className="jb-btn-primary"
                    style={svcStyles.quoteBtn}
                    onClick={e => { e.stopPropagation(); setScreen('contact'); }}
                  >Get a Quote →</button>
                ) : (
                  <span className="jb-learn-more" style={svcStyles.learnMore}>Learn more →</span>
                )}
              </div>
            </div>
          );
      ...
      {/* ── Bold CTA strip ── */}
      <div style={svcStyles.ctaStrip}>
        <div style={svcStyles.ctaInner}>
          <div style={svcStyles.ctaBolt}>
            <img src={window.ASSETS?.['bolt-icon.png'] || 'bolt-icon.png'} alt="" style={{ width: 56, height: 'auto', display:'block' }}/>
          </div>
          <div>
            <div style={svcStyles.ctaTitle}>Ready to get started?</div>
            <div style={svcStyles.ctaSub}>All estimates are completely free. Call or request online.</div>
          </div>
          <div style={svcStyles.ctaActions}>
            <button
              className="jb-btn-primary"
              style={svcStyles.ctaBtnPrimary}
              onClick={() => setScreen('contact')}
            >Request Free Quote</button>
            <a href="tel:9783979878" className="jb-btn-ghost" style={svcStyles.ctaBtnSecondary}>978.397.9878</a>
          </div>
        </div>
      </div>
    </div>
